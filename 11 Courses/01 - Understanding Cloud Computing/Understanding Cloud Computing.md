---
tags:
  - cloud
status: Completed
start_date: 2025-11-23
end_date: 2025-11-28
---
# Course: Understanding Cloud Computing

## 1. Summary & Goals

_Every day we interact with the cloud—whether it’s using Google Drive, apps like Salesforce, or accessing our favorite websites. Cloud computing has become the norm for many companies, but what exactly is the cloud and why is everyone rushing to adopt it? Designed for absolute novices, this course breaks down what the cloud is and explains terminology such as scalability, latency, and high-availability. You'll learn about the many advantages including ease of remote collaboration, how there are no hardware limitations, and reliable disaster recovery. You'll also discover the range of tools provided by major cloud providers such as Amazon Web Services (AWS), Microsoft Azure, and Google Cloud. By the end of this course, you'll be able to confidently explain how cloud tools can increase productivity and save money, as well as ask the right questions about how to optimize your use of cloud tools._

## 2. Core Concepts & Notes

_This is the central index for this course. As I create new atomic notes, I will link them here under the relevant module or topic._

### Chapter 1: Introduction to Cloud Computing

**Introduction to cloud computing**
- [[Cloud - What is Cloud Computing]]
- [[Cloud - Major Cloud Providers]]
- [[Cloud - On-Premise Servers]]
- [[Cloud - Cloud Hosting]]
- [[Cloud - On-Premise vs Cloud Hosting]]
- [[Cloud - Pay-As-You-Go Pricing Model]]
- [[Cloud - Core Characteristics]]
- [[Cloud - Common Use Cases]]
- [[Cloud - DataCamp Traffic Spike Example]]

**The power of the cloud**
- [[Cloud - Core Service Offerings]]
- [[Cloud - Compute Services]]
- [[Cloud - Storage Services]]
- [[Cloud - Database Services]]
- [[Cloud - Key Advantages]]
- [[Cloud - Virtualization]]
- [[Cloud - Cost Efficiency]]
- [[Cloud - Scalability]]
- [[Cloud - Vertical vs Horizontal Scaling]]
- [[Cloud - Pay-as-You-Go Pricing]]
- [[Cloud - Speed and Performance]]
- [[Cloud - Reliability]]
- [[Cloud - Security]]

**Cloud service models**
- [[Cloud - Cloud Service Models]]
- [[Cloud - Infrastructure as a Service (IaaS)]]
- [[Cloud - Platform as a Service (PaaS)]]
- [[Cloud - Software as a Service (SaaS)]]
- [[Cloud - IaaS vs PaaS vs SaaS]]
- [[Cloud - Cloud Service Models & Control vs Convenience Relationship]]
- [[Cloud - Function as a Service (FaaS)]]
- [[Cloud - Anything as a Service (XaaS)]]

### Chapter 2: Cloud Deployment

**Cloud deployment models**
- [[Cloud - Cloud Deployment Models]]
- [[Cloud - Private Cloud]]
- [[Cloud - Public Cloud]]
- [[Cloud - Hybrid Cloud]]
- [[Cloud - Cloud Bursting]]
- [[Cloud - Multicloud]]
- [[Cloud - Community Cloud]]
- [[Cloud - Private Cloud vs On-Premise Infrastructure]]
- [[Cloud - Hybrid Cloud vs Multicloud]]

**Regulations on the cloud**
- [[Cloud - Data Sovereignty & Cloud Computing]]
- [[Cloud - Latency]]
- [[Cloud - Global Data Centers & Latency Relationship]]
- [[Cloud - General Data Protection Regulation (GDPR)]]
- [[Cloud - GDPR Consent Requirement]]
- [[Cloud - GDPR Data Breach Notification]]
- [[Cloud - GDPR Data Protection Principles]]
- [[Cloud - GDPR Data Residency & Transfer Rules]]
- [[Cloud - GDPR Penalties]]
- [[Cloud - Personal Data (PII)]]

**Cloud Computing Roles**
- [[Cloud - Demand for Cloud Computing Skills]]
- [[Cloud - Provider Certifications]]
- [[Cloud - Impact on Data Roles]]
- [[Cloud - Data Scientist Cloud Usage]]
- [[Cloud - Machine Learning Scientist Cloud Usage]]
- [[Cloud - Data Engineer Cloud Usage]]
- [[Cloud - Data Analyst Cloud Usage]]
- [[Cloud - Collaboration Benefits]]
- [[Cloud - Emergent Job Roles]]
- [[Cloud - Cloud Architect Role]]
- [[Cloud - Cloud Engineer Role]]
- [[Cloud - DevOps]]
- [[Cloud - DevOps Engineer Role]]
- [[Cloud - Security Engineer Role]]
- [[Cloud - Cloud Architect & Cloud Engineer Relationship]]

### Chapter 3: Cloud Providers and Case Studies

**An Overview of Providers**
- [[Cloud - Cloud Provider Landscape Overview]]
- [[Cloud - Provider Market Share]]
- [[Cloud - Business Benefits of IaaS & PaaS]]
- [[Cloud - Cloud Provider Selection Process]]
- [[Cloud - Provider Selection Criteria]]

**Amazon Web Services**
- [[Cloud - Amazon Web Services (AWS)]]
- [[Cloud - AWS History & Market Position]]
- [[Cloud - AWS Simple Storage Service (S3)]]
- [[Cloud - AWS Elastic Compute Cloud (EC2)]]
- [[Cloud - AWS Relational Database Service (RDS)]]
- [[Cloud - AWS Redshift]]
- [[Cloud - AWS Kinesis]]
- [[Cloud - AWS SageMaker]]
- [[Example - NerdWallet AWS SageMaker Implementation]]

**Microsoft Azure**
- [[Cloud - Microsoft Azure]]
- [[Cloud - Azure Key Service Offerings]]
- [[Cloud - Azure Blob Storage]]
- [[Cloud - Azure Virtual Machines]]
- [[Cloud - Azure SQL Database]]
- [[Cloud - Microsoft Fabric]]
- [[Cloud - Azure Data Services]]
- [[Cloud - Azure Data Lake Storage]]
- [[Cloud - Azure Stream Analytics]]
- [[Cloud - Azure Machine Learning]]
- [[Cloud - Azure Notable Clients]]
- [[Example - The Ottawa Hospital Azure Implementation]]
- [[Cloud - Disaster Recovery]]
- [[Cloud - Azure Site Recovery]]
- [[Cloud - Personal Health Information Protection Act (PHIPA)]]

**Google Cloud**
- [[Cloud - Google Cloud Platform (GCP)]]
- [[Cloud - Google Cloud Anthos]]
- [[Cloud - Google Cloud Storage]]
- [[Cloud - Google Cloud Compute Engine]]
- [[Cloud - Google Cloud SQL]]
- [[Cloud - BigQuery]]
- [[Cloud - Dataflow]]
- [[Cloud - AutoML]]
- [[Example - Lush Migration to GCP]]

****
## 3. Key Takeaways & Reflections

_After completing the course, what are the 2-3 most important ideas that I will carry forward? Were there any "aha!" moments that connected this topic to others in a new way?_

## 4. Related Projects & Applications

_A list of projects where I have applied the skills from this course. This directly links my theoretical knowledge to practical, portfolio-worthy application._

- [[ ]]

---
## To-Do

### Chapter 1: Introduction to Cloud Computing

- [ ] **Introduction to cloud computing**
	- [x] [[Cloud - What is Cloud Computing]]
	- [x] [[Cloud - Major Cloud Providers]]
	- [x] [[Cloud - On-Premise Servers]]
	- [x] [[Cloud - Cloud Hosting]]
	- [x] [[Cloud - On-Premise vs Cloud Hosting]]
	- [x] [[Cloud - Pay-As-You-Go Pricing Model]]
	- [x] [[Cloud - Core Characteristics]]
	- [ ] [[Cloud - Common Use Cases]]
	- [ ] [[Cloud - DataCamp Traffic Spike Example]]

- [ ] **The power of the cloud**
	- [ ] [[Cloud - Core Service Offerings]]
	- [ ] [[Cloud - Compute Services]]
	- [ ] [[Cloud - Storage Services]]
	- [ ] [[Cloud - Database Services]]
	- [ ] [[Cloud - Key Advantages]]
	- [ ] [[Cloud - Virtualization]]
	- [ ] [[Cloud - Cost Efficiency]]
	- [ ] [[Cloud - Scalability]]
	- [ ] [[Cloud - Vertical vs Horizontal Scaling]]
	- [ ] [[Cloud - Pay-as-You-Go Pricing]]
	- [ ] [[Cloud - Speed and Performance]]
	- [ ] [[Cloud - Reliability]]
	- [ ] [[Cloud - Security]]

- [ ] **Cloud service models**
	- [ ] [[Cloud - Cloud Service Models]]
	- [ ] [[Cloud - Infrastructure as a Service (IaaS)]]
	- [ ] [[Cloud - Platform as a Service (PaaS)]]
	- [ ] [[Cloud - Software as a Service (SaaS)]]
	- [ ] [[Cloud - IaaS vs PaaS vs SaaS]]
	- [ ] [[Cloud - Cloud Service Models & Control vs Convenience Relationship]]
	- [ ] [[Cloud - Function as a Service (FaaS)]]
	- [ ] [[Cloud - Anything as a Service (XaaS)]]

### Chapter 2: Cloud Deployment

- [ ] **Cloud deployment models**
	- [ ] [[Cloud - Cloud Deployment Models]]
	- [ ] [[Cloud - Private Cloud]]
	- [ ] [[Cloud - Public Cloud]]
	- [ ] [[Cloud - Hybrid Cloud]]
	- [ ] [[Cloud - Cloud Bursting]]
	- [ ] [[Cloud - Multicloud]]
	- [ ] [[Cloud - Community Cloud]]
	- [ ] [[Cloud - Private Cloud vs On-Premise Infrastructure]]
	- [ ] [[Cloud - Hybrid Cloud vs Multicloud]]

- [ ] **Regulations on the cloud**
	- [ ] [[Cloud - Data Sovereignty & Cloud Computing]]
	- [ ] [[Cloud - Latency]]
	- [ ] [[Cloud - Global Data Centers & Latency Relationship]]
	- [ ] [[Cloud - General Data Protection Regulation (GDPR)]]
	- [ ] [[Cloud - GDPR Consent Requirement]]
	- [ ] [[Cloud - GDPR Data Breach Notification]]
	- [ ] [[Cloud - GDPR Data Protection Principles]]
	- [ ] [[Cloud - GDPR Data Residency & Transfer Rules]]
	- [ ] [[Cloud - GDPR Penalties]]
	- [ ] [[Cloud - Personal Data (PII)]]

- [ ] **Cloud Computing Roles**
	- [ ] [[Cloud - Demand for Cloud Computing Skills]]
	- [ ] [[Cloud - Provider Certifications]]
	- [ ] [[Cloud - Impact on Data Roles]]
	- [ ] [[Cloud - Data Scientist Cloud Usage]]
	- [ ] [[Cloud - Machine Learning Scientist Cloud Usage]]
	- [ ] [[Cloud - Data Engineer Cloud Usage]]
	- [ ] [[Cloud - Data Analyst Cloud Usage]]
	- [ ] [[Cloud - Collaboration Benefits]]
	- [ ] [[Cloud - Emergent Job Roles]]
	- [ ] [[Cloud - Cloud Architect Role]]
	- [ ] [[Cloud - Cloud Engineer Role]]
	- [ ] [[Cloud - DevOps]]
	- [ ] [[Cloud - DevOps Engineer Role]]
	- [ ] [[Cloud - Security Engineer Role]]
	- [ ] [[Cloud - Cloud Architect & Cloud Engineer Relationship]]

### Chapter 3: Cloud Providers and Case Studies

- [ ] **An Overview of Providers**
	- [ ] [[Cloud - Cloud Provider Landscape Overview]]
	- [ ] [[Cloud - Provider Market Share]]
	- [ ] [[Cloud - Business Benefits of IaaS & PaaS]]
	- [ ] [[Cloud - Cloud Provider Selection Process]]
	- [ ] [[Cloud - Provider Selection Criteria]]

- [ ] **Amazon Web Services**
	- [ ] [[Cloud - Amazon Web Services (AWS)]]
	- [ ] [[Cloud - AWS History & Market Position]]
	- [ ] [[Cloud - AWS Simple Storage Service (S3)]]
	- [ ] [[Cloud - AWS Elastic Compute Cloud (EC2)]]
	- [ ] [[Cloud - AWS Relational Database Service (RDS)]]
	- [ ] [[Cloud - AWS Redshift]]
	- [ ] [[Cloud - AWS Kinesis]]
	- [ ] [[Cloud - AWS SageMaker]]
	- [ ] [[Example - NerdWallet AWS SageMaker Implementation]]

- [ ] **Microsoft Azure**
	- [ ] [[Cloud - Microsoft Azure]]
	- [ ] [[Cloud - Azure Key Service Offerings]]
	- [ ] [[Cloud - Azure Blob Storage]]
	- [ ] [[Cloud - Azure Virtual Machines]]
	- [ ] [[Cloud - Azure SQL Database]]
	- [ ] [[Cloud - Microsoft Fabric]]
	- [ ] [[Cloud - Azure Data Services]]
	- [ ] [[Cloud - Azure Data Lake Storage]]
	- [ ] [[Cloud - Azure Stream Analytics]]
	- [ ] [[Cloud - Azure Machine Learning]]
	- [ ] [[Cloud - Azure Notable Clients]]
	- [ ] [[Example - The Ottawa Hospital Azure Implementation]]
	- [ ] [[Cloud - Disaster Recovery]]
	- [ ] [[Cloud - Azure Site Recovery]]
	- [ ] [[Cloud - Personal Health Information Protection Act (PHIPA)]]

- [ ] **Google Cloud**
	- [ ] [[Cloud - Google Cloud Platform (GCP)]]
	- [ ] [[Cloud - Google Cloud Anthos]]
	- [ ] [[Cloud - Google Cloud Storage]]
	- [ ] [[Cloud - Google Cloud Compute Engine]]
	- [ ] [[Cloud - Google Cloud SQL]]
	- [ ] [[Cloud - BigQuery]]
	- [ ] [[Cloud - Dataflow]]
	- [ ] [[Cloud - AutoML]]
	- [ ] [[Example - Lush Migration to GCP]]
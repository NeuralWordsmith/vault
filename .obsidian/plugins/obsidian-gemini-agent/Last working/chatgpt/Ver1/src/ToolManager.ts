...
import { segmentIntoUnits, buildPlanningInputFromSource } from "./segment";
...
// === SEGMENTER HELPERS ===
function shouldSplit(sentence: string): boolean {
  const STRONG = [
  ...
  const VISUAL = [
    ...
    /\bfit(ting)? a? linear regression\b/i, /\bcoefficients?\b/i,
    /\btrain(?:ing)?\/?test\b/i, /\b80\/20\b/i
  ];
  const tests = [...STRONG, ...EXAMPLE, ...CONTRAST];
  ...
  if (VISUAL.some(r => r.test(sentence))) return true;
  return false;
}

function segmentIntoUnits(source: string): string[] {
...
function buildPlanningInputFromSource(sourceText: string): string {
  const units = segmentIntoUnits(sourceText);
  return [
    "UNITS (each ≈ one slide-level concept; do NOT change my naming rules):",
    ...units.map((u, i) => `Unit ${i + 1}: ${u}`),
    "",
    "SOURCE:",
    sourceText
  ].join("\n");
}
// === END SEGMENTER HELPERS ===
...
export class ToolManager {
    ...
    private async _debugLog(message: string) {
        const logFileName = `_GeminiDebugLog.md`;
        const logFilePath = `${this.settings.draftsFolderPath}/${logFileName}`;
        const timestamp = new Date().toISOString();

        // Append or create fresh log if missing
        const file = this.fileService.getAbstractFileByPath(logFilePath);
        if (file instanceof TFile) {
            await this.fileService.appendToFile(logFilePath, `\n${timestamp} — ${message}`);
        } else {
            await this.fileService.writeFile(logFilePath, `# Gemini Debug Log\n\n${timestamp} — ${message}`);
        }
    }
    ...
    public async createNotePlan(targetFilePath: string | null, progressCallback: (update: string) => void): Promise<string> {
        try {
            ...
            // Send to AI exactly as before, but with planningInput
            const planningInput = buildPlanningInputFromSource(rawContent);
            await this._debugLog(`Planning Input:\n${planningInput}`);
            const planData = await this._fetchAndParsePlanFromAI(planningInput, vaultContext);
    ...
    private async _extractNoteContent(targetFile: TFile): Promise<{ rawContent: string | null, extractedSource: string }> {
        // 1) Read & normalize line endings (defensive for Windows files)
        const fullContentRaw = await this.fileService.readFile(targetFile);
        const fullContent = fullContentRaw.replace(/\r\n/g, "\n");

        // 2) Capture EVERYTHING under "# Source" until the next heading or EOF
        // - case-insensitive for "Source"
        // - multiline so ^ matches line starts
        const sourceBlockRegex = /^#\s*Source\s*\n([\s\S]*?)(?=\n^#\s|\Z)/mi;
        const sourceBlockMatch = fullContent.match(sourceBlockRegex);

        // 3) If not found, bail early
        if (!sourceBlockMatch) {
            await this._debugLog(`Source not found under "# Source".`);
            return { rawContent: null, extractedSource: '' };
        }

        let rawContent = sourceBlockMatch[1].trim();

        // 4) Optional: extract a 'source - ...' line ONLY if present; don't override content
        // Anchored to start-of-line to avoid accidental matches in body paragraphs.
        const sourceMetaRegex = /^source\s*-\s*(.+)$/im;
        const sourceMetaMatch = rawContent.match(sourceMetaRegex);
        const extractedSource = sourceMetaMatch ? sourceMetaMatch[1].trim().replace(/^- /, '') : '';

        // Remove that single meta line from the content shown to the AI (non-destructive)
        if (sourceMetaMatch) {
            rawContent = rawContent.replace(sourceMetaRegex, '').trim();
        }

        // 5) Debug logs so we can spot truncation instantly
        await this._debugLog(`Source length: ${rawContent.length} chars`);
        await this._debugLog(`Extracted # Source text (head 200):\n${rawContent.slice(0, 200)}`);
        await this._debugLog(`Extracted # Source text (tail 200):\n${rawContent.slice(-200)}`);
    ...
    private async _fetchAndParsePlanFromAI(rawContent: string, vaultContext: string): Promise<any> {
        ...
        const rawResponse = result.response.text();
        await this._debugLog(`Raw Gemini Response:\n${rawResponse}`);
        ...
        try {
            ...
            // ✅ Log success
            await this._debugLog(`Parsed Plan JSON:\n${JSON.stringify(parsed, null, 2)}`);

            return parsed;
        } catch (parseError) {
            console.error("Failed to parse JSON from Gemini:", rawResponse);

            // ✅ Log failure case too
            await this._debugLog(`❌ JSON Parse Error: ${parseError}\nRaw text was:\n${rawResponse}`);

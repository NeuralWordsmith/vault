...
- **Details Object Logic**: You MUST structure the detailed explanation within a single JSON key called "details". This key MUST contain an object with the following five sub-keys: "core_idea", "primary_goal", "mechanism_bullets", "parameters_bullets", and "tradeoffs_bullets".  
    - "core_idea": Your goal is to build intuition for a beginner. Use simple language, avoid jargon, and focus on the 'big idea' to defining the concept's fundamental mechanism, after that, define what is it and what is it a part of and if it has its own parts further.
...
- **Connection Logic**: Your most important task is to create a rich, interconnected knowledge graph. For any "connection" field, do not just list the links. Instead, you MUST write a descriptive sentence that explains the relationship between the concepts, embedding the wikilink within that sentence.
    - seemlesly include concept's relationships to other concepts in easy language(e.g., "**part of**", "**prerequisite for**", "**solves problem**" , "**impacts**", "**example of**", "**contrasts with**", "**built upon**", "**alternative to**").  This is the most important part of your task.
...
`;
    ...
    public async generateNotesFromPlan(plan_file_path: string | undefined, progressCallback: (update: string) => void): Promise<string> {
        ...
        try {
            ...
            if (failedNotes.length > 0) {
                // We use .replace to clean up the "(failed)" text for a cleaner log
                logMessage += `\n- **Failed:**` + failedNotes.map(r => `\n\t- ${r.link.replace(' (failed)', '')}`).join("");
            }

            await this.logActivity(logMessage);

            let returnMessage = `Successfully generated ${generatedCount} of ${proposals.length} notes in ${durationInSeconds} seconds.`;
            if (failedNotes.length > 0) {
                returnMessage += ` (${failedNotes.length} failed). Check the log for details.`;
            }
    ...
    public async generateCodeLabNotes(folderPath: string, progressCallback: (update: string) => void): Promise<string> {
        ...
        // First, pre-filter the notes to get an accurate total count
        for (const note of allNotes) {
            const content = await this.fileService.readFile(note);
            if (content.match(implementationLinkRegex)) {
    ...
    private async _processProposals(proposals: RegExpMatchArray[], mainTopic: string, sourceFromPlan: string[], progressCallback: (update: string) => void) {
        ...
        for (const [index, proposal] of proposals.entries()) {
            ...
            if (type === 'Visual Analysis') {
                try {
                    // Directly call the refactored function with the content from the plan.
                    const result = await this.analyzeVisualsInNote(context, title);
                    new Notice(result, 7000); // Show success/error message
                    results.push({
                        success: true,
                        link: `[[${this.settings.draftsFolderPath}/${title}.md|${title}]]`, // Construct the link
                        isStructural: false
                    });
                } catch (e) {

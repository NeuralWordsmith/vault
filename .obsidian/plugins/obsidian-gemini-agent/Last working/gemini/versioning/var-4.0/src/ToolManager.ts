...
export class ToolManager {
    ...
    new Notice("Scanning vault for structural notes...", 3000);
        ...
        for (const file of files) {
            ...
            // It now handles tags with or without a leading '#'
...
`;
    ...
    async generateNotesFromPlan(plan_file_path: string | undefined, progressCallback: (update: string) => void): Promise<string> {
        ...
        for (const proposal of proposals) {
            ...
            let templatePath = "";
            // 1. First, try to find a template that dynamically matches the note's type.
            //    (e.g., for type 'ComparativeAnalysis', it looks for 'ComparativeAnalysis Template.md')
            ...
            // 2. If no dynamic match is found, fall back to the old hard-coded list for backwards compatibility.
            else {
            ...
            // 3. If no template was found by either method, skip the note.
            if (!templatePath) {

...
import { makeApiCallWithRetry } from './Utils';
    ...
    async run(userInput: string, view: AgentChatView, mode: 'command' | 'chat', activeFilePath: string | null) {
        ...
        try {
            if (mode === 'chat') {
                ...
                const result = await makeApiCallWithRetry(() => model.generateContent(userInput), 3, 2000, progressCallback);
                ...
                while (true) {
                    ...
                    result = await makeApiCallWithRetry(() => this.chat.sendMessage(JSON.stringify(toolResults)), 3, 2000, progressCallback);
                }
            ...
            // Check if this error happened after a generation task

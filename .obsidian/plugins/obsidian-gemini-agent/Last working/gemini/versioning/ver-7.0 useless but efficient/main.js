...
var ToolManager = class {
  ...
  async makeApiCallWithRetry(apiCall, progressCallback, maxRetries = 3, initialDelay = 2e3) {
    let retries = 0;
    ...
    while (retries < maxRetries) {
      try {
        ...
        if (error.message && error.message.includes("[503")) {
          ...
          new import_obsidian.Notice(message, waitTime * 1e3);
...
`;
  ...
  async createNotePlan(targetFilePath, progressCallback) {
    ...
    if (!(targetFile instanceof import_obsidian.TFile)) return `Error: Target file not found at path "${targetFilePath}".`;
...
\`\`\``;
      ...
      if (templatesFolder instanceof import_obsidian.TFolder) {
        templateFileNames = templatesFolder.children.filter((file) => file instanceof import_obsidian.TFile && file.extension === "md").map((file) => file.name);
      }
  ...
  async generateNotesFromPlan(plan_file_path, progressCallback) {
    ...
    for (const [noteIndex, proposal] of proposals.entries()) {
      ...
      try {
        ...
        new import_obsidian.Notice(`Error generating note: ${title}. Check console for details.`, 1e4);
...
Successfully generated ${generatedCount} notes before stopping. You can try running the command again in a few minutes.`;
    ...
    if (newStructuralNotes.length > 0) {
      const mapFilePath = `${this.settings.draftsFolderPath}/${HIERARCHY_MAP_FILE_NAME}`;
      const mapFile = this.fileService.getAbstractFileByPath(mapFilePath);
      if (mapFile instanceof import_obsidian.TFile) {
  ...
  async reviewAnswersInPlan(fileToUpdate) {
    ...
    try {
      ...
      const keywordModel = this.genAI.getGenerativeModel({ model: this.settings.model, generationConfig: { temperature: 0 } });
...
<!-- gemini-context: ${keywordText} -->`;
      ...
      new import_obsidian.Notice(`Error getting feedback. The API might be overloaded. Please try again later.`, 7e3);
...
// src/Utils.ts
async function makeApiCallWithRetry(apiCall, maxRetries = 3, initialDelay = 2e3, progressCallback) {
  let retries = 0;
  let delay = initialDelay;
  while (retries < maxRetries) {
    try {
      ...
      if (error.message && /\[5\d{2}\]/.test(error.message)) {
        ...
        const waitSeconds = (totalDelay / 1e3).toFixed(1);
  ...
  async run(userInput, view, mode, activeFilePath) {
    ...
    try {
      if (mode === "chat") {
        ...
        const result = await makeApiCallWithRetry(() => model.generateContent(userInput), 3, 2e3, progressCallback);
        ...
        while (true) {
          ...
          result = await makeApiCallWithRetry(() => this.chat.sendMessage(JSON.stringify(toolResults)), 3, 2e3, progressCallback);
        }
        view.removeThinkingMessage();
        view.addMessage("model", result.response.text());
      }

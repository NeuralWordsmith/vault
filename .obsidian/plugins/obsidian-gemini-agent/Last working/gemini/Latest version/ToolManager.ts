...
- **Hierarchical Context**
    ...
    - If filling "connections_related" or "connections_other_fundamentals", you must intelligently identify and link to other relevant concepts, describing the nature of the relationship.
...
- **VALID JSON IS REQUIRED**: All backslashes (\`\\\`) in your response, especially those in LaTeX commands, MUST be escaped with another backslash. For example, write \`"$$\\\\lambda \\\\sum" \` instead of \`"$$\\lambda \\sum"\`.
...
`;
    ...
    private populateTemplate(templateContent: string, data: Record<string, any>): string {
        ...
        for (const key in data) {
            ...
            if (Array.isArray(replacement)) {
                if (key.toLowerCase() === 'tags_yaml') {
                    // --- ADD THIS NEW CASE ---
                    // Special formatting for the YAML tags list
                    replacement = "\n" + replacement.map(item => `  - ${String(item)}`).join("\n");

                } else if (key.toLowerCase() === 'related') {
                    // Special formatting for YAML related list (with wikilinks)
                    ...
                    // Generic fallback for simple bulleted lists in the note body
        ...
        // Remove any leftover placeholders that didn't have data
    ...
    async generateNotesFromPlan(plan_file_path: string | undefined, progressCallback: (update: string) => void): Promise<string> {
        ...
        for (const proposal of proposals) {
            ...
            try {
                ...
                // Then, use a Set to automatically remove any duplicates
                const uniqueTags = [...new Set(allTags)];

                // Format the clean, unique array into a YAML list string
                ...
                // --- 3. ADD THIS LINE ---
                // Add this line to format and inject the source

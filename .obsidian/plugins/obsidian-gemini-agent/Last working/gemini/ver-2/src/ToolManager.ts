...
export class ToolManager {
    ...
    async analyzeVisualsInNote(noteFile: TFile): Promise<string> {
        const fullNoteContent = await this.fileService.readFile(noteFile);

        const blockRegex = /```gemini-note\n([\s\S]*?)\n```/;
        const blockMatch = fullNoteContent.match(blockRegex);

        if (!blockMatch || !blockMatch[1]) {
            return "Error: No `gemini-note` block found to analyze.";
        }
        ...
        if (imageNames.length === 0) {
            return "Error: No image links found inside the `gemini-note` block.";
        }

        let analysisTitle = "";
        if (imageNames.length === 1) {
            analysisTitle = `Visual Analysis of ${imageNames[0]}`;
        } else {
            analysisTitle = `Analysis of Visual Sequence`;
        }
        ...
        try {
            ...
            // Pass the blockContent as context, not the full note
    ...
    public async createNotePlan(targetFilePath: string | null, progressCallback: (update: string) => void): Promise<string> {
        try {
            ...
            if (!rawContent) return "Error: The note or 'gemini-note' block is empty.";
    ...
    /**
     * Reads a file and extracts the raw content from the gemini-note block
     * (or the whole file as a fallback) and any source metadata.
     */
    private async _extractNoteContent(targetFile: TFile): Promise<{ rawContent: string | null, extractedSource: string }> {
        ...
        const geminiBlockRegex = /```gemini-note\n([\s\S]*?)\n```/;
        const geminiBlockMatch = fullContent.match(geminiBlockRegex);
        let rawContent = geminiBlockMatch ? geminiBlockMatch[1] : fullContent;
...
`;
    ...
    /**
     * Assembles all the pieces of the plan (feedback, proposals, questions) into a single string.
     */
    private _assemblePlanContent(planData: any, extractedSource: string, templateDraftNotification: string): string {
        const feedbackSection = `## Feedback & Synthesis\n---\n\n${planData.overall_feedback}\n\n` +
            planData.review_points.map((p: any) => `- **${p.concept}:** ${p.suggestion}`).join('\n');
...
`;
        return [frontmatter, feedbackSection, relatedConnectionsSection, templateDraftNotification, proposalSection, notesPlanSection, questionsSection, synthesisSection].join('');
    }
    ...
    public async extractMechanismsForPlan(folderPath: string, progressCallback: (update: string) => void): Promise<string> {
        ...
        // --- THIS REGEX IS NOW FIXED (removed the extra space in the lookahead) ---
        ...
        if (extractedCount === 0) {
            return "Scan complete. No notes with mechanism sections found in that folder.";
        }
...
- **Details Object Logic**: You MUST structure the detailed explanation within a single JSON key called "details". This key MUST contain an object with the following five sub-keys: "core_idea", "primary_goal", "mechanism_bullets", "parameters_bullets", and "tradeoffs_bullets".  
    - "core_idea": Provide a single, powerful sentence defining the concept's fundamental mechanism.
    - "primary_goal": Explain the specific problem the concept solves and who primarily benefits from this solution.
    - "mechanism_bullets": Provide a step-by-step explanation. Each main step should have a bolded title. If an explanation introduces a defining formula, you MUST **append the formula to the end of the same 'content' string**. Use a double newline (\\n\\n) to create a paragraph break before the formula, which MUST be formatted with double dollar signs ("$$...$$"). For example: "content": "The final objective function becomes:\\n\\n$$J_{reg}(\\theta) = J(\\theta) + \\lambda\\Omega(\\theta)$$".
    - "mechanism_code_snippet": This is a CRITICAL field for practical understanding. If the mechanism can be illustrated with a concise Python code block (e.g., implementing a loss function, a simple forward pass), mark comments with # translating steps from mechanism_bullets you MUST provide it here. If the concept is purely theoretical or doesn't have a clear code equivalent, you MUST return an empty string ("").
    - "parameters_bullets": Identify key hyperparameters using the nested bullet point structure.
    - "tradeoffs_bullets": Explain the consequences of using this concept (e.g., Bias-Variance Tradeoff) using the nested bullet point structure.
`;
...
- **Hierarchical Context**
    ...
    - If filling "connections_other_fundamentals", you MUST provide an array of sentences describing how this concept connects to other broad, foundational topics (e.g., "Fundamental - Statistics", "Fundamental - Computer Science"). You must use aliased wikilinks in your description.
...
You MUST return a single, valid JSON object that strictly adheres to the following nested structure. All fields are mandatory.

{
  ...
  "details": {
    ...
    "tradeoffs_bullets": "Explain the consequences and impact on tradeoffs (e.g., Bias-Variance) using the nested bullet point structure."
  },
  "connections": {
...
`;
    ...
    private formatNestedList(items: any[], level = 0): string {
        ...
        for (const item of items) {
            ...
            if (isFormula && result.length > 0) {
                ...
                // This is a regular text item. Create a normal bullet point for it.
    ...
    public async generateNotesFromPlan(plan_file_path: string | undefined, progressCallback: (update: string) => void): Promise<string> {
        ...
        try {
            ...
            const generatedCount = generationResults.filter(r => r.success).length;
            
            await this.logActivity(
                `Generated ${generatedCount} notes in ${durationInSeconds}s from plan "[[${plan_file_path}]]":` + 
                generationResults.filter(r => r.success).map(r => `\n\t- ${r.link}`).join("")
            );

            return `Successfully generated ${generatedCount} of ${proposals.length} notes in ${durationInSeconds} seconds.`;
    ...
    public async generateCodeLabNotes(folderPath: string, progressCallback: (update: string) => void): Promise<string> {
        ...
        // Define the regex to find the specific implementation wikilink.
        // This looks for [[FILENAME|View Full Implementation & Analysis]]
        const implementationLinkRegex = /\[\[(.*?)\|View Full Implementation & Analysis\]\]/;

        // We no longer need to pre-filter, we can check for the link inside the loop.
        for (const [index, note] of allNotes.entries()) {
            const content = await this.fileService.readFile(note);
            
            // --- NEW LOGIC: Find the link and extract the filename ---
            const linkMatch = content.match(implementationLinkRegex);

            // If the note doesn't have our specific link, skip it.
            if (!linkMatch) {
                continue;
            }

            // The captured part (the filename) is in the first group of the match.
            const newNoteFileName = `${linkMatch[1]}.md`;
            
            const conceptName = note.basename.replace(/^Core: /, '').trim();
            progressCallback(`[${index + 1}/${allNotes.length}] Found target link in: ${conceptName}...`);
        ...
        const finalMessage = generatedCount > 0 
            ? `Successfully generated ${generatedCount} Code Lab notes!`
            : "Scan complete. No notes with the required implementation link were found.";
            
        await this.logActivity(`Generated ${generatedCount} Code Lab notes from "${folderPath}".`);
        return finalMessage;
    }
    ...
    public async generateMathNotesFromPlan(planFilePath: string, progressCallback: (update: string) => void): Promise<string> {
        ...
        const durationInSeconds = ((Date.now() - startTime) / 1000).toFixed(1);
        const logMessage = `Generated ${generatedCount} atomic math notes in ${durationInSeconds}s from plan "[[${planFilePath}]]".`;
        await this.logActivity(logMessage);

        return `Successfully generated ${generatedCount} of ${tasks.length} math notes in ${durationInSeconds} seconds!`;
    }
    ...
    private async _processProposals(proposals: RegExpMatchArray[], mainTopic: string, sourceFromPlan: string[], progressCallback: (update: string) => void) {
        ...
        for (const [index, proposal] of proposals.entries()) {
            ...
            progressCallback(`[${index + 1}/${proposals.length}] Generating: ${title}...`);
            ...
            try {
                ...
                if (e instanceof z.ZodError) {
                    console.error(`Zod validation error for "${title}":`, e.errors);
